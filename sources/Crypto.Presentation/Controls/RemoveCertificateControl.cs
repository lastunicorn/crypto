...
public class RemoveCertificateControl : SectionControlBase
{
    ...
    public RemoveCertificateControl(RemoveCertificateResult result)
    {
        Title = result.Title;
        CertificateSubjectName = result.CertificateSubjectName;
        CertificateContainsPk = result.CertificateContainsPrivateKey;
        PkExists = result.PrivateKeyFileInfo != null;
        PkFilePath = result.PrivateKeyFileInfo?.FilePath;
        PkDirectoryPath = result.PrivateKeyFileInfo?.DirectoryPath;
        PkLocationType = result.PrivateKeyFileInfo?.LocationType.ToString();
        PkWasRemoved = result.PrivateKeyFileInfo?.IsSuccessfullyRemoved ?? false;
        PkRemoveError = result.PrivateKeyFileInfo?.RemoveError;
        Error = result.Error;
    }
